...
output "s3_information" {
  value = {
    s3_source = aws_s3_bucket.source.bucket
    s3_dest = aws_s3_bucket.destination.bucket
  }
}

output "test_upload_command" {
  description = "Command to test the S3 → Lambda pipeline"
  value       = "aws s3 cp test.txt s3://${aws_s3_bucket.source.bucket}/test.md"
}

output "check_destination_bucket" {
  description = "Command to verify the file in the destination bucket"
  value       = "aws s3 ls s3://${aws_s3_bucket.destination.bucket}/"
}
