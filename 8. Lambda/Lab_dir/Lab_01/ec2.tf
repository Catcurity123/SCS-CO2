...
resource "aws_instance" "lab01_bastion_host" {
  ...
  vpc_security_group_ids = [aws_security_group.lab01_bastion_sg.id]
  ...
  tags = {
    Name = "lalab01-amazon-linux-2-public"
  }
}

#"ssh -i ${local_file.private_key_pem.filename} ec2-user@${aws_instance.bastion_host.public_ip}"
# ssh -i bastion-key.pem ec2-user@3.84.2.114