...
apt update -y
apt upgrade -y
apt install -y nginx python3 unzip wget curl gnupg2 software-properties-common git build-essential

systemctl start nginx
systemctl enable nginx

# 2. Fetch an IMDSv2 token (valid for 6 hours)
TOKEN=$(curl -sX PUT "http://169.254.169.254/latest/api/token" \
             -H "X-aws-ec2-metadata-token-ttl-seconds: 21600")

# 3. Create the HTML page displaying EC2 metadata
...
# 5. Use CloudWatch config from SSM
