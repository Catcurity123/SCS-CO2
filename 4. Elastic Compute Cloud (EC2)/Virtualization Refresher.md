...
(+) With this method the hardware is `virtaulization-awared`, `SR-IOV` is a mechanism that virtualizes a single PCIe Ethernet controller to make it appear as multiple PCIe devices. 

(+) A hardware-level feature that enables high-performance networking by allowing a single network interface card (NIC) to appear as multiple virtual NICs to the guest operating systems running on the instance

(+) **Physical Functions (PFs) and Virtual Functions (VFs):** An SR-IOV capable NIC has Physical Functions (PFs) which are full PCIe functions that manage the SR-IOV functionality and can also move data. It also has Virtual Functions (VFs), which are lightweight PCIe functions that primarily handle data transfer. 

(+)  **Direct Access:**: SR-IOV allows virtual machines to directly access these VFs, bypassing the hypervisor's software switch. This direct access significantly reduces latency and overhead, leading to improved network performance. 


![[Pasted image 20240108154112.png]]

![[Pasted image 20250808142619.png]]
